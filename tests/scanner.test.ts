...
describe("scanRepository", () => {
  ...
  it("skips marked Repo Context CLI outputs without hiding hand-authored files nearby", async () => {
    const root = await createTempRepo();
    await mkdir(join(root, ".ai-context"), { recursive: true });
    await writeFile(join(root, "package.json"), "{\"name\":\"generated-skip-app\"}", "utf8");
    await writeFile(
      join(root, "AGENTS.md"),
      "<!-- repo-context-cli:generated; Generated by Repo Context CLI. -->\n# Agent Instructions\n",
      "utf8"
    );
    await writeFile(
    ...
    await writeFile(join(root, ".ai-context", "index.json"), "{\"generatedBy\":\"Repo Context CLI\",\"schemaVersion\":1}\n", "utf8");
    await writeFile(join(root, ".ai-context", "notes.md"), "# Hand-authored notes\n", "utf8");

    const result = await scanRepository(root, {
      maxFiles: 100,
      generatedFilePaths: ["AGENTS.md", "PROJECT_MAP.md", ".ai-context/index.json"]
    });

    expect(result.files.map((file) => file.path)).toEqual([".ai-context/notes.md", "package.json"]);
  });
});
